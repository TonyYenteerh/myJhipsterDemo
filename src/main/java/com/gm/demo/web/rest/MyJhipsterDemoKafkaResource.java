...
import com.gm.demo.config.KafkaSseConsumer;
import com.gm.demo.config.KafkaSseProducer;
...
@RestController
@RequestMapping("/api/my-jhipster-demo-kafka")
public class MyJhipsterDemoKafkaResource {

    private final Logger log = LoggerFactory.getLogger(MyJhipsterDemoKafkaResource.class);

    private final MessageChannel output;
    private Sinks.Many<Message<String>> sink = Sinks.many().unicast().onBackpressureBuffer();

    public MyJhipsterDemoKafkaResource(@Qualifier(KafkaSseProducer.CHANNELNAME) MessageChannel output) {
        this.output = output;
    }
